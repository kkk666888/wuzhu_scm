...
export default {
  ...
  methods: {
                ...
                <div>
                  {
                    <el-button
                      type="primary"
                      disabled={confirmReceiptBtnDisable}
                      onClick={() => {
                        thisObj.confirmReceiptOpen(param.row);
                      }}
                    >
                      确认收货
                    </el-button>
...

                  {
                    <el-button
                      type="primary"
                      disabled={cancellationBtnDisable}
                      onClick={() => {
                        thisObj.cancellationOpen(param.row);
                      }}
                    >
                      作废
                    </el-button>
...

                  {
                    <el-button
                      ...
                      disabled={true}
                      onClick={() => {
                        thisObj.qualityTestingOpen(param.row);
                      }}
                    >
                      发起质检
                    </el-button>
    ...
    confirmReceiptSave() {
      ...
      this.api.returnApplyNote.updateReturnApplyNote.send(this.confirmReceiptModel, { showLoading: true }).then(res => {
        if (res.code === '00') {
    ...
    cancellationSave() {
      ...
      this.api.returnApplyNote.updateReturnApplyNote.send(this.cancellationModel, { showLoading: true }).then(res => {
        if (res.code === '00') {
